...
namespace LaCabana
{
	...
	public class BasicMapDemoActivity : BaseActionActivity, IOnMapReadyCallback, ILocationListener
	{
		...
		protected override void OnCreate(Bundle bundle)
		{
			...
			ConstructRightIcon();
			SetTitleActionBar1(GetString(Resource.String.map));
			SetProfilePicture();
			ClickHandler();
		...
		public void GetData()
		{
			...
			{
				CreateDialog(GetString(Resource.String.Error), GetString(Resource.String.Unabletoload), false, "", true, GetString(Resource.String.Cancel), false);
			}
			RunOnUiThread(() =>
			{
				PutAllMarker();
